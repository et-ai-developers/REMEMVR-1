...
def loadMaster():
    ...
    if os.path.exists('./data/cache/dfMaster.csv'):   

        print("Cached Version Found")

        # dfMaster = pd.read_pickle('./data/cache/dfMaster.db')
        dfMaster = read_csv_data('./data/cache/dfMaster.csv')
        ...
        dfMaster = pd.read_excel('./data/master.xlsx')

        print("Creating New Cache")

        cache_files = os.listdir('./data/cache')
        for file in cache_files:
            os.remove(f'./data/cache/{file}')

        # pd.to_pickle(dfMaster, "./data/cache/dfMaster.db")
        save_csv_data('./data/cache/dfMaster.csv', dfMaster)
...
def loadVariables():
    ...
    dfVariables = pd.read_excel('./data/variables.xlsx', sheet_name='variables')

    if os.path.exists('./data/cache/dfVariables.csv'):   

        print("Cached Version Found")

        # dfVariablesCache = pd.read_pickle('./data/cache/dfVariables.db')   
        dfVariablesCache = read_csv_data('./data/cache/dfVariables.csv')
        ...
        if dfVariables.equals(dfVariablesCache):
            ...
            # pd.to_pickle(dfVariables, "./data/cache/dfVariables.db")
            save_csv_data('./data/cache/dfVariables.csv', dfVariables)
...
def loadData(dfMaster, dfVariables):
    ...
    if os.path.exists('./data/cache/dfData.csv'):

        print("Cached Version Found")

        # dfData = pd.read_pickle('./data/cache/dfData.db')
        dfData = read_csv_data('./data/cache/dfData.csv')
        ...
        # pd.to_pickle(dfData, "./data/cache/dfData.db")
        save_csv_data('./data/cache/dfData.csv', dfData)
...
def create_df_with_custom_variables():
    pass