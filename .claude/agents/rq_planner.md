...
#### B. Read Project-Specific Statistical Insights

**Action:** Read docs/project_specific_stats_insights.md during ultrathink

**Purpose:** Apply mandatory project-wide decisions

**Critical Decisions to Apply:**
...
**Example Step (2-Pass IRT, Step 1 - Pass 1 Calibration):**

```markdown
## Step 1: IRT Calibration Pass 1 (All Items)

**Dependencies:** Step 0 (data extraction) complete

**Complexity:** High (30-60 minutes for N=100, 102 items, 4 tests)

**Input:**
- File: data/irt_input.csv
- Format: Wide (composite_ID × item columns)
- Dimensions: 400 rows (100 participants × 4 tests) × 103 cols (composite_ID + 102 items)
- Required Columns: composite_ID, VR-*-A01-O-ANS (102 temporal item columns)

**Processing:**
- Method: Graded Response Model (GRM) calibration
- Model: Multidimensional IRT with 3 dimensions (common, congruent, incongruent)
- Item Scoring: Dichotomous (0 = incorrect, 1 = correct)
- Dimension Mapping:
  - Dimension 1 (common): i1CM, i2CM items
  - Dimension 2 (congruent): i3CG, i4CG items
  - Dimension 3 (incongruent): i5IN, i6IN items
- Epochs: 1000 (standard for GRM convergence)

**Output:**
- File 1: logs/pass1_item_params.csv
  - Columns: item_name, dimension, discrimination (a), difficulty (b), se_a, se_b
  - Dimensions: 102 rows (items) × 6 cols
- File 2: logs/pass1_theta.csv
  - Columns: composite_ID, theta_common, theta_congruent, theta_incongruent, se_common, se_congruent, se_incongruent
  - Dimensions: 400 rows × 7 cols

**Validation Requirements (MANDATORY):**

**Methodological Criteria (for validation tools during execution):**
- Model convergence (gradient norm < 0.01, all parameters stable)
- Fit indices (RMSEA < 0.08, CFI > 0.90, TLI > 0.90)
- Local independence (Q3 < 0.2 for all item pairs)
- Validation tools MUST be used after analysis tool execution (specific tools by rq_tools)

**Substance Criteria (for rq_inspect post-execution validation):**

*Output Files:*
- pass1_item_params.csv: 102 rows × 6 columns (item_name: object, dimension: object, a: float64, b: float64, se_a: float64, se_b: float64)
- pass1_theta.csv: 400 rows × 7 columns (composite_ID: object, theta_common: float64, theta_congruent: float64, theta_incongruent: float64, se_common: float64, se_congruent: float64, se_incongruent: float64)

*Value Ranges:*
- theta values ∈ [-3, 3] (outside range indicates calibration problem)
- se (standard errors) ∈ [0.1, 1.0] (above 1.0 = unreliable estimates, below 0.1 = suspiciously low)
- discrimination (a) > 0 (negative discrimination impossible)
- difficulty (b): no restriction (temporal items can have |b| > 5.0 per D039)

*Data Quality:*
- All 400 composite_IDs present (100 participants × 4 tests = 400, no data loss)
...
- Item count: exactly 102 items in item_params (no items dropped during calibration)

*Log Validation:*
- Log file: logs/step01_irt_pass1.log must exist
- Required pattern: "Model converged: True" (or "Convergence achieved")
- Required patterns: "VALIDATION - PASS: theta range check", "VALIDATION - PASS: se range check"
- Forbidden patterns: "ERROR", "EXCEPTION", "CONVERGENCE FAILED", "VALIDATION - FAIL"
- Acceptable warnings: "Temporal items have extreme difficulty |b| > 3.0 (expected per D039)"
...
**Expected Behavior:**
- GRM calibration converges within 1000 epochs (typical: 300-600 epochs)
- Item parameters estimated for all 102 items (no missing)
- Theta scores estimated for all 400 observations (composite_IDs)
- Some temporal items show extreme difficulty (|b| > 3.0) - expected, documented in D039
- Standard errors reasonable (median se_theta ~0.3-0.5 typical for N=100)
```

**Example Step (2-Pass IRT, Step 2 - Purification):**

```markdown
## Step 2: Item Purification (Decision D039)

**Dependencies:** Step 1 (Pass 1 calibration) complete with valid item parameters

**Complexity:** Low (<5 minutes - filtering operation)

**Input:**
- File: logs/pass1_item_params.csv
- Required Columns: item_name, discrimination (a), difficulty (b)

**Processing:**
- Method: 2-Pass IRT purification per Decision D039
- Thresholds:
  - Discrimination: a ≥ 0.4 (items must discriminate ability)
  - Difficulty: |b| ≤ 3.0 (items not too easy or too hard)
- Exclusion: Items failing EITHER threshold removed
- Expected Retention: 40-50% (temporal items challenging, many exceed |b| > 3.0)

**Output:**
- File: data/purified_items.csv
  - Columns: item_name, dimension, a, b (retained items only)
  - Dimensions: ~43 rows (42% of 102 items) × 4 cols
- File: logs/purification_report.txt
  - Lists excluded items with reasons (low discrimination or extreme difficulty)

**Validation Requirements (MANDATORY):**

**Methodological Criteria (for validation tools during execution):**
- At least 10 items retained per dimension (minimum for stable IRT estimation)
- No dimension completely eliminated (all 3 dimensions must have items)
- Retention rate within expected bounds (30-60% for temporal items per D039)
- Validation tools MUST be used after purification (specific tools by rq_tools)

**Substance Criteria (for rq_inspect post-execution validation):**

*Output Files:*
- purified_items.csv: ~43 rows × 4 columns (item_name: object, dimension: object, a: float64, b: float64)
- purification_report.txt: text file listing excluded items with reasons

*Value Ranges:*
- discrimination (a) ≥ 0.4 (by definition - purification threshold)
- difficulty (b): |b| ≤ 3.0 (by definition - purification threshold)
- Retention rate: 30-60% (outside range may indicate problem with thresholds)

*Data Quality:*
- Row count: 30-60 items (42% of 102 = 43 items expected, but range acceptable)
- All retained items have valid a and b (no NaN values)
- No duplicate item_names (each item appears once)
- Dimension distribution: At least 10 items per dimension (minimum for stable estimation)
...
- Forbidden patterns: "ERROR", "Dimension eliminated (0 items)"
- Acceptable warnings: "Low retention rate [X]% (expected 40-50% for temporal items per D039)"
...
**Expected Behavior:**
- Temporal items (~42 items) likely excluded (|b| > 3.0 common for memory items per D039)
- ~43 items retained (42% retention typical, range 30-60% acceptable)
- All 3 dimensions have sufficient items (minimum 10 per dimension, typical 12-16 per dimension)
- Excluded items documented in purification_report.txt with reasons (discrimination too low OR difficulty too extreme)
```
...
**Context Dump Structure (3 pieces per spec):**
...
**Keep context_dump terse:** ~3 sentences, high-level summary for other agents to read.
...
### Example 1: Simple IRT-Only Analysis (RQ 5.8 - Temporal Memory Difficulty)

**Concept Summary:**
- Analyze difficulty of temporal domain items (When: -O-)
- All paradigms, acquisition items only (A01)
- IRT calibration with item parameter extraction
- No LMM (descriptive only)

**Your Output (2_plan.md excerpt):**

```markdown
# Analysis Plan: Temporal Memory Difficulty (RQ 5.8)

**Pipeline:** IRT only (descriptive item analysis)
**Steps:** 4 total
**Estimated Runtime:** Medium (~90 minutes)
...
**Input:** master.xlsx
**Tags:** VR-*-A01-O-ANS (temporal items, all paradigms, acquisition)
**Expected Dimensions:** 100 participants × 4 tests × 102 items = 400 rows, 103 cols
**Format:** Wide (composite_ID as row, item tags as columns)
**Output:** data/irt_input.csv

**Validation:** All 100 participants present, all 4 tests present, 102 item columns extracted
...
**Complexity:** High (30-60 min)

**Input:** data/irt_input.csv (400 × 103)
**Processing:** Graded Response Model (GRM), unidimensional (temporal ability)
**Output:** logs/pass1_item_params.csv (102 items), logs/pass1_theta.csv (400 observations)

**Validation Requirements (MANDATORY):**
- Convergence: All item parameters stable within 1000 epochs
- Fit: RMSEA < 0.08, CFI > 0.90
- Local Independence: Q3 statistic < 0.2
- Tools: Validation tools MUST be used after analysis tool execution
- On Failure: Quit, log error, invoke g_debug

**Expected:** Many temporal items will have |b| > 3.0 (high difficulty common)
...
**Input:** logs/pass1_item_params.csv
**Processing:** Filter items by thresholds (|b| ≤ 3.0, a ≥ 0.4)
**Output:** data/purified_items.csv (~43 items, 42% retention expected)

**Validation Requirements (MANDATORY):**
- Minimum 10 items retained
- Tools: Validation tools MUST be used
- On Failure: If <10 items, report (may need threshold adjustment)
...
**Complexity:** Medium (15-30 min, fewer items than Pass 1)

**Input:** data/irt_input.csv filtered to purified items
**Processing:** GRM calibration on purified item set
**Output:** data/item_parameters.csv (final), data/theta_scores.csv (final)

**Validation Requirements (MANDATORY):**
- Convergence: Improved or equal to Pass 1
- Fit: RMSEA improved vs Pass 1
- Reliability: Theta standard errors < 0.5
- Tools: Validation tools MUST be used
- On Failure: Quit, log, invoke g_debug
...
**Data Files:**
- data/irt_input.csv (extraction)
- data/purified_items.csv (purification)
- data/item_parameters.csv (final IRT parameters)
- data/theta_scores.csv (final ability estimates)

**Logs:**
- logs/pass1_item_params.csv (Pass 1 diagnostics)
- logs/purification_report.txt (exclusion summary)
- logs/validation_results.txt (all validation checks)
...
| Step | Analysis | Validation Criteria |
|------|----------|---------------------|
| 0 | Extraction | 100 participants, 4 tests, 102 items |
| 1 | IRT Pass 1 | Convergence, fit (RMSEA<0.08), Q3<0.2 |
| 2 | Purification | ≥10 items retained |
| 3 | IRT Pass 2 | Convergence, improved fit, SE<0.5 |
```

---

### Example 2: IRT→LMM Trajectory Analysis (RQ 5.1 - Temporal Forgetting)

**Concept Summary:**
- Trajectory of temporal memory forgetting (Days 0, 1, 3, 6)
- IRT theta scores as outcome in LMM
- Group comparison: common vs congruent vs incongruent items
- Dual-scale plots for interpretability

**Your Output (2_plan.md excerpt):**

```markdown
# Analysis Plan: Temporal Memory Forgetting Trajectory (RQ 5.1)

**Pipeline:** IRT → LMM (longitudinal trajectory)
**Steps:** 8 total
**Estimated Runtime:** High (~150 minutes)

**Decisions Applied:**
- D039: 2-pass IRT purification
- D068: Dual p-value reporting (group comparisons)
- D069: Dual-scale trajectory plots
- D070: TSVR as LMM time variable

---

## Analysis Plan

[Steps 0-3: Same as Example 1 - IRT calibration with 2-pass purification]

---

### Step 4: Merge Theta with TSVR (Decision D070)

**Dependencies:** Step 3 complete (theta_scores.csv exists)
**Complexity:** Low (<5 min)

**Input:**
- data/theta_scores.csv (400 rows: 100 participants × 4 tests)
- master.xlsx TSVR lookup (Time Since VR in hours)

**Processing:**
- Parse composite_ID → extract UID + test
- Lookup TSVR_hours from master.xlsx per UID × test
- Merge theta scores with TSVR (NOT nominal days 0, 1, 3, 6)
- Reshape wide → long format (one row per observation)

**Output:**
- data/lmm_input.csv (400 rows in long format)
- Columns: UID, test, TSVR_hours, theta_common, theta_congruent, theta_incongruent

**Validation Requirements (MANDATORY):**
- All 400 composite_IDs successfully matched to TSVR
- No missing TSVR values
- TSVR range check (0-168 hours for 7-day span)
- Tools: Validation tools MUST be used
- On Failure: Quit if any TSVR missing (data integrity issue)

---

### Step 5: Fit LMM with TSVR Time Variable (Decision D070)

**Dependencies:** Step 4 complete
**Complexity:** Medium (15-30 min)

**Input:** data/lmm_input.csv (400 observations in long format)

**Processing:**
- Model: Linear Mixed Model
- Fixed Effects: TSVR_hours (time), dimension (common/congruent/incongruent), TSVR × dimension interaction
- Random Effects: (1 + TSVR_hours | UID) - random intercept + random slope per participant
- Time Variable: TSVR_hours (actual hours since encoding, NOT days)

**Output:**
- results/lmm_model_summary.txt (fixed effects, random effects, fit indices)
- results/lmm_residuals.csv (for diagnostic plotting)

**Validation Requirements (MANDATORY):**
- Residual normality: Shapiro-Wilk p > 0.05
- Homoscedasticity: Levene test p > 0.05
- No autocorrelation: Durbin-Watson statistic 1.5-2.5
- Model convergence: All parameters stable
- Tools: Validation tools MUST be used
- On Failure: Quit, log assumption violations, invoke g_debug

---

### Step 6: Post-Hoc Contrasts (Decision D068)

**Dependencies:** Step 5 complete (LMM converged)
**Complexity:** Low (<5 min)

**Input:** LMM fitted model from Step 5

**Processing:**
- Pairwise comparisons: common vs congruent, common vs incongruent, congruent vs incongruent
- Multiple testing: Bonferroni correction (3 comparisons, α = 0.05/3 = 0.0167)
- Report BOTH uncorrected AND corrected p-values (Decision D068)
- Compute effect sizes: Cohen's d, partial η²

**Output:**
- results/post_hoc_contrasts.csv (comparisons, both p-values, effect sizes)

**Validation Requirements (MANDATORY):**
- All pairwise comparisons completed (3 total)
- Both p-value columns present (uncorrected, Bonferroni)
...
**Dependencies:** Step 5 (LMM) complete, Step 3 (theta scores) complete
**Complexity:** Low (<5 min)

**Input:**
- data/theta_scores.csv (from Step 3, theta estimates per composite_ID)
- data/observed_means.csv (from Step 5, aggregated observed data with CIs)
- data/tsvr_mapping.csv (from Step 4, time since VR per test session)

**Processing:**
- Merge theta scores + observed means + TSVR on composite_ID and test
- Select columns needed for trajectory plotting: time, theta, CI_lower, CI_upper, domain
- Create TWO source CSVs (Decision D069 - dual-scale trajectory plots):
  - Theta-scale data: plots/trajectory_theta_data.csv
  - Probability-scale data: plots/trajectory_probability_data.csv (same data, different transformation applied during plotting)

**Output:**
- File 1: plots/trajectory_theta_data.csv
  - Columns: time (TSVR hours), theta, CI_lower, CI_upper, domain
  - Dimensions: 400 rows (100 participants × 4 tests) × 5 columns
  - Data types: time: float64, theta: float64, CI_lower: float64, CI_upper: float64, domain: object
- File 2: plots/trajectory_probability_data.csv
  - Same structure as theta_data.csv (transformation to probability scale happens in plot_trajectory_probability function)
  - Dimensions: 400 rows × 5 columns

**Validation Requirements (MANDATORY):**

**Methodological Criteria:**
- All composite_IDs successfully merged (no data loss)
- TSVR values within expected range (0 to ~168 hours for 7-day study)
- Validation tools MUST be used after data aggregation

**Substance Criteria (for rq_inspect):**

*Output Files:*
- trajectory_theta_data.csv: 400 rows × 5 columns (time: float64, theta: float64, CI_lower: float64, CI_upper: float64, domain: object)
- trajectory_probability_data.csv: 400 rows × 5 columns (same schema)

*Value Ranges:*
- time ∈ [0, 168] (TSVR hours, 7-day study)
- theta ∈ [-3, 3] (IRT latent ability)
- CI_lower < CI_upper (confidence interval consistency)
- CI_lower ≥ -3, CI_upper ≤ 3 (within plausible theta range)

*Data Quality:*
- All 400 composite_IDs present (100 participants × 4 tests, no data loss)
- No NaN values in time, theta, CI columns
- domain column has 3 unique values: common, congruent, incongruent

*Log Validation:*
- Required: "Merge successful: 400/400 rows matched"
- Forbidden: "WARNING: Data loss during merge"

**Plot Source CSV Preparation (Option B):**

*Plot 1 Description:*
- "Theta-scale trajectory plot showing memory ability over time with 95% confidence bands, grouped by domain (common, congruent, incongruent)"
- Plot type: Trajectory

*Required Data Sources:*
- theta_scores.csv, observed_means.csv, tsvr_mapping.csv (merged)
...
*Plotting Function:*
- "trajectory plot" (rq_plots will map to plot_trajectory)

*Plot 2 Description (Decision D069):*
- "Probability-scale trajectory plot showing memory performance as % correct over time, grouped by domain"
- Plot type: Trajectory (probability-scale)

*Source CSV Path:*
- plots/trajectory_probability_data.csv

*Plotting Function:*
- "trajectory plot probability-scale" (rq_plots will map to plot_trajectory_probability)

**NOTE:** This step creates plot SOURCE CSVs. Actual PNG generation happens later when rq_plots reads these CSVs and calls tools/plotting.py functions.
...
**Data Files:**
- data/irt_input.csv (Step 0)
- data/purified_items.csv (Step 2)
- data/theta_scores.csv (Step 3)
- data/lmm_input.csv (Step 4)

**Results Files:**
- results/lmm_model_summary.txt (Step 5)
- results/post_hoc_contrasts.csv (Step 6, D068 dual p-values)
- results/effect_sizes.csv (Step 6)

**Plot Source CSVs (Option B - created during analysis, PNGs generated later by rq_plots):**
- plots/trajectory_theta_data.csv (Step 7, source data for theta-scale plot)
- plots/trajectory_probability_data.csv (Step 7, source data for probability-scale plot, D069)
...
| 2 | Purification | ≥10 items retained |
| 3 | IRT Pass 2 | Convergence, improved fit, SE<0.5 |
| 4 | TSVR Merge | All composite_IDs matched, TSVR range valid |
| 5 | LMM Fit | Normality, homoscedasticity, no autocorrelation |
| 6 | Post-hoc | 3 comparisons, dual p-values (D068), effect sizes |
| 7 | Plot Data Prep | Source CSVs created with required columns, D069 compliance (both theta + probability CSVs) |
```
