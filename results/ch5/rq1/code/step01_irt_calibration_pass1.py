...
def format_item_params_for_output(
    ...
    We need to create: item, domain, Discrimination, Difficulty_1
    ...
    Returns:
        Formatted DataFrame with [item, domain, Discrimination, Difficulty_1]
    """
    ...
    for _, row in df_items.iterrows():
        ...
        output_rows.append({
            'item': item_name,
            'domain': domain,
            'Discrimination': row['Overall_Discrimination'],
            'Difficulty_1': row['Difficulty']
        })
...
if __name__ == "__main__":
    try:
        ...
        log(f"[SUMMARY]   Discrimination range: {df_items_formatted['Discrimination'].min():.3f} - {df_items_formatted['Discrimination'].max():.3f}")
        log(f"[SUMMARY]   Difficulty range: {df_items_formatted['Difficulty_1'].min():.3f} - {df_items_formatted['Difficulty_1'].max():.3f}")
        for domain in ['what', 'where', 'when']:
            n_domain = len(df_items_formatted[df_items_formatted['domain'] == domain])
        ...
        a_min, a_max = df_items_formatted['Discrimination'].min(), df_items_formatted['Discrimination'].max()
        ...
        b_min, b_max = df_items_formatted['Difficulty_1'].min(), df_items_formatted['Difficulty_1'].max()
        ...
        nan_items = df_items_formatted[['Discrimination', 'Difficulty_1']].isna().any(axis=1).sum()
