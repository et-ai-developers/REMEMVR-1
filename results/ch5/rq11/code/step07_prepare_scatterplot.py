...
the three memory domains (What, Where, When).

EXPECTED INPUTS:
  - data/step00_irt_theta_loaded.csv
    Columns: ['composite_ID', 'theta_what', 'theta_where', 'theta_when']
    Format: IRT theta scores from RQ 5.1 Pass 2 (wide format)
  ...
  - data/step01_ctt_scores.csv
    ...
    Format: CTT mean scores per domain (long format)
...
if __name__ == "__main__":
    try:
        ...
        log("[RESHAPE] Converting IRT theta from wide to long format...")
        ...
        # Filter out 'Overall' domain (only need what, where, when for individual scatter plots)
        correlations_subset = correlations[correlations['domain'].isin(['what', 'where', 'when'])][['domain', 'r']].copy()
        ...
        # Validates: All 3 domains present, expected row count, no missing data,
        ...
        if actual_rows != expected_rows:
            ...
            log(f"[PASS] Row count: {actual_rows} (matches expected)")
