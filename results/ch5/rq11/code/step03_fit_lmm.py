...
EXPECTED INPUTS:
  - data/step00_irt_theta_loaded.csv
    ...
    Format: Wide-format IRT theta scores from RQ 5.1
  ...
  - data/step01_ctt_scores.csv
    ...
    Format: Long-format CTT mean scores per domain
    Expected rows: ~1200 (400 participant-tests × 3 domains)
...
- Approach: Fit identical LMM formula to IRT and CTT scores separately,
  ...
  Random: TSVR_hours | UID (attempt random slopes, fallback to intercepts-only)

- Why this approach: Decision D070 requires TSVR (actual hours) as time variable.
  Dual time predictors (linear + log) capture both immediate drop and gradual decay.
  Domain interactions test if What/Where/When domains show different trajectories.
  Parallel structure ensures fair IRT vs CTT comparison.

- Data flow:
  IRT wide (theta_what, theta_where, theta_when) -> long (domain, IRT_score)
  CTT already long (domain, CTT_score)
...
if __name__ == "__main__":
    try:
        ...
        log("[PREPARE] Reshaping IRT theta to long format...")

        # Reshape IRT theta from wide to long
        # Wide: composite_ID, theta_what, theta_where, theta_when
        ...
        # Map theta column names to domain names
        domain_mapping = {
            'theta_what': 'What',
            'theta_where': 'Where',
            'theta_when': 'When'
        ...
        log(f"[RESHAPED] IRT long format ({len(irt_long)} rows)")
        ...
        if 'composite_ID' not in ctt_scores.columns:
            ctt_scores['composite_ID'] = ctt_scores['UID'] + '_' + ctt_scores['test']
        ...
        # Define formulas
        fixed_formula = "score ~ (TSVR_hours + log_TSVR) * C(domain)"
        re_formula_slopes = "TSVR_hours"  # Random slopes + intercepts
        re_formula_intercepts = "1"  # Intercepts-only
