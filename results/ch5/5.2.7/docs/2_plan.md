...
This RQ examines whether participants can be grouped into latent classes based on domain-specific forgetting trajectories across three episodic memory domains (What, Where, When). The analysis uses K-means clustering on 6 clustering variables per participant: intercept and slope for each domain derived from RQ 5.2.6.
...
- Expected input: results/ch5/5.2.6/data/step04_random_effects.csv (300 rows: 100 UID x 3 domains)
...
**Format:** CSV, long format with 300 rows (100 participants x 3 domains)
**Required Columns:**
...
**Expected Dimensions:** 300 rows x 4 columns

**Filters:**
- No filters applied - all 100 participants x 3 domains required for clustering
...
3. Validate structure: 300 rows (100 UID x 3 domains), 4 columns (UID, domain, Total_Intercept, Total_Slope)
4. Pivot from long to wide format:
   - Rows: 100 UIDs
   - Columns: 6 clustering variables
     ...
     - Total_Intercept_When (When domain baseline)
     - Total_Slope_When (When domain forgetting rate)
5. Verify no missing values (all 100 participants must have all 6 variables)
...
**Columns:**
  ...
  - `Total_Intercept_When` (float, When domain baseline theta)
  - `Total_Slope_When` (float, When domain forgetting rate)
**Expected Rows:** 100 participants
**Expected Columns:** 7 (UID + 6 clustering variables)
...
- Expected columns: 7 (UID + 6 clustering variables)
- Data types: UID (object/string), all 6 clustering variables (float64)
...
- No NaN values (all 6 clustering variables must be non-null for all participants)
- No duplicate UIDs (each participant appears exactly once)
- Pivot validation: 100 UID x 3 domains (300 rows in source) -> 100 UID x 6 variables (wide output)

*Log Validation:*
- Required pattern: "Loaded 300 rows from RQ 5.2.6"
- Required pattern: "Pivoted to wide format: 100 rows x 6 clustering variables"
...
**Purpose:** Standardize all 6 clustering variables to z-scores (mean=0, SD=1) to ensure equal weighting across domains and parameters in K-means distance calculations.

**Input:**

**File:** data/step00_random_effects_from_rq526.csv
**Source:** Generated by Step 0 (pivoted random effects)
**Format:** CSV, wide format with 100 rows x 7 columns
**Required Columns:**
  - `UID` (string)
  - 6 clustering variables (Total_Intercept_What, Total_Slope_What, Total_Intercept_Where, Total_Slope_Where, Total_Intercept_When, Total_Slope_When)

**Processing:**

1. Load step00_random_effects_from_rq526.csv
2. For each of 6 clustering variables:
...
**Columns:**
  - `UID` (string)
  - 6 z-scored clustering variables:
    ...
    - `Total_Intercept_When_z` (float, standardized When baseline)
    - `Total_Slope_When_z` (float, standardized When slope)
**Expected Rows:** 100 participants
**Expected Columns:** 7 (UID + 6 z-scored variables)
...
**Contents:**
  - Mean and SD for each of 6 raw variables (before standardization)
  - Mean and SD for each of 6 z-scored variables (after standardization)
...
*Log Validation:*
- Required pattern: "Standardized 6 variables to z-scores"
...
**Format:** CSV with 100 rows x 7 columns (UID + 6 z-variables)
**Clustering Matrix:** Extract 6 z-scored columns only (exclude UID column)
**Expected Shape:** 100 participants x 6 features (numpy array for sklearn)

**Processing:**

1. Load standardized features (100 x 6 matrix, z-scored)
...
**Format:** 100 participants x 6 z-scored features
...
1. Load standardized features (100 x 6 z-scored matrix)
...
5. Extract cluster centers:
   - cluster_centers_ attribute (K x 6 matrix of z-scored centroids)
...
**Format:** CSV with K rows (one per cluster), 7 columns
**Columns:**
  - `cluster` (int, cluster ID 0 to K-1)
  - 6 z-scored centroid coordinates:
    ...
    - `Total_Intercept_When_z` (float, When baseline centroid)
    - `Total_Slope_When_z` (float, When slope centroid)
...
- data/step03_cluster_centers.csv exists
  ...
  - Expected columns: 7 (cluster ID + 6 centroid coordinates)
...
**Format:** 100 participants x 6 z-scored variables
...
**Format:** 100 participants x 7 columns (UID + 6 variables in original theta scale)
...
2. For each cluster (0 to K-1):
   - Compute mean and SD for each of 6 variables:
     ...
     - Total_Intercept_When (When baseline)
     - Total_Slope_When (When forgetting rate)
...
3. Domain-specific pattern identification:
   ...
   - Identify domain-selective patterns:
     - Example: Cluster 1 = High What intercept, Low Where/When intercepts
     - Example: Cluster 2 = Fast What forgetting, Slow Where/When forgetting
4. Assign interpretive labels per cluster:
   - Label format: "{Baseline pattern} / {Forgetting pattern}"
   - Examples:
     - "High Memory / Slow Forgetting" (high all intercepts, shallow all slopes)
     - "Domain-Selective What" (high What intercept, average Where/When)
     - "Fast Temporal Decline" (steep When slope, moderate What/Where slopes)
...
**Format:** CSV with K x 6 = K*6 rows (one row per cluster per variable), 6 columns
**Columns:**
  - `cluster` (int, cluster ID 0 to K-1)
  - `variable` (string, one of 6 clustering variables)
...
**Expected Rows:** K * 6 (e.g., if K=3: 18 rows)
...
- data/step05_cluster_summary_statistics.csv exists
  - Expected rows: K * 6 (e.g., K=3 -> 18 rows)
...
- variable in {Total_Intercept_What, Total_Slope_What, Total_Intercept_Where, Total_Slope_Where, Total_Intercept_When, Total_Slope_When}
...
- All 6 variables present per cluster (K rows per variable)
...
**Format:** 100 participants x 7 columns (UID + 6 z-variables)
...
**Format:** K clusters x 7 columns (cluster ID + 6 z-scored centroids)
...
2. Create plot data matrix:
   ...
   - Columns:
     ...
     - 6 z-scored variables (axes for pairwise scatter plots):
...
3. Create cluster centers annotation data:
   ...
   - Columns: Same as plot data (cluster ID, label, 6 z-coordinates)
...
**Format:** CSV with (100 + K) rows, 10 columns
**Columns:**
  ...
  - 6 z-scored variables (float, axes for scatter plot matrix):
    - `Total_Intercept_What_z`
    - `Total_Slope_What_z`
    - `Total_Intercept_Where_z`
    - `Total_Slope_Where_z`
...
**Note:** rq_plots will create 6x6 pairwise scatter plot matrix (15 unique pairs) with:
...
- data/step06_scatter_plot_matrix_data.csv exists
  ...
  - Expected columns: 10 (UID, cluster, cluster_label, point_type, 6 z-variables)
...
**Plot Description:** 6x6 scatter plot matrix showing pairwise relationships among 6 clustering variables, with points colored by cluster assignment and cluster centroids marked.
...
- Dimensions: 12x12 inches @ 300 DPI (high resolution for publication)
...
- data/step00_random_effects_from_rq526.csv (100 rows x 7 cols: UID + 6 raw-scale clustering variables from RQ 5.2.6)
- data/step01_standardized_features.csv (100 rows x 7 cols: UID + 6 z-scored clustering variables)
...
- data/step03_cluster_centers.csv (K rows x 7 cols: cluster ID + 6 z-scored centroids)
...
- data/step05_cluster_summary_statistics.csv (K*6 rows: cluster stats per variable)
- data/step05_cluster_characterization.txt (text report: interpretive cluster descriptions)
- data/step06_scatter_plot_matrix_data.csv (100+K rows x 10 cols: plot source data)
...
- plots/step06_cluster_scatter_matrix.png (created by rq_plots, NOT by analysis steps)
  ...
  - 6x6 scatter plot matrix colored by cluster with centroids marked
...
- Input: 300 rows (100 UID x 3 domains in long format)
- Output: 100 rows (UID as rows, 6 domain-specific variables as columns)
- Transformation: pivot_wider(names_from=domain, values_from=c(Total_Intercept, Total_Slope))

**Step 1 -> Step 2 (Standardization):**
- Input: 100 x 6 raw-scale matrix
- Output: 100 x 6 z-scored matrix
...
- Output: 100 rows merged (UID, 6 variables, cluster)
...
- Possible patterns:
  - Global high/average/low memory (uniform across domains)
  - Domain-selective impairment (poor spatial only, poor temporal only)
  - Dissociations between What vs Where/When (dual-process systems)
