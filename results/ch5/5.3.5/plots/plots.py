...
# -*- coding: utf-8 -*-
"""
Plotting script for RQ 5.3.5: IRT-CTT Convergence for Paradigm-Specific Forgetting

GENERATED BY: Master (matching 5.2.1 style)
DATE: 2025-12-03
PURPOSE: Create publication-ready plots from individual-level data

PLOT STYLE (matching RQ 5.2.1):
...
PLOTS GENERATED:
1. scatterplot_irt_ctt.png - IRT theta vs CTT mean by paradigm
2. trajectory_irt.png - IRT theta trajectories by paradigm
3. trajectory_ctt.png - CTT mean trajectories by paradigm
...
print("=" * 70)
print("RQ 5.3.5: IRT-CTT Convergence - Plotting (5.2.1 Style)")
print("=" * 70)
...
print("\n" + "-" * 70)
print("Generating Plot 1: IRT vs CTT Scatterplot by Paradigm...")
print("-" * 70)
...
for paradigm in ['IFR', 'ICR', 'IRE']:
    ...
    paradigm_data = irt_input[irt_input['paradigm'] == paradigm]
    ax.scatter(
    ...
    paradigm_pred = irt_pred_grid[irt_pred_grid['paradigm'] == paradigm]

    # 3. Plot shaded CI band
    ax.fill_between(
        paradigm_pred['TSVR_hours'],
        paradigm_pred['ci_lower'],
        paradigm_pred['ci_upper'],
        color=color,
        alpha=MEAN_CI_ALPHA,
        edgecolor='none'
    )
...
ax.set_title('RQ 5.3.5: Paradigm-Specific Forgetting Trajectories - IRT Theta Scale', fontsize=14, fontweight='bold')
...
for paradigm in ['IFR', 'ICR', 'IRE']:
    ...
    # 1. Plot faded scatter points (individual observations)
    paradigm_data = ctt_input[ctt_input['paradigm'] == paradigm]
    ax.scatter(
        paradigm_data['TSVR_hours'],
        paradigm_data['CTT_mean'] * 100,  # Convert to percentage
        c=color,
        alpha=RESIDUAL_SCATTER_POINT_ALPHA,
        s=15,
        edgecolors='none'
    ...
    # 2. Get predictions for this paradigm
    paradigm_pred = ctt_pred_grid[ctt_pred_grid['paradigm'] == paradigm].copy()

    # 3. Plot shaded CI band (convert to percentage)
    ax.fill_between(
        paradigm_pred['TSVR_hours'],
        paradigm_pred['ci_lower'] * 100,
        paradigm_pred['ci_upper'] * 100,
        color=color,
        alpha=MEAN_CI_ALPHA,
        edgecolor='none'
    )
...
ax.set_title('RQ 5.3.5: Paradigm-Specific Forgetting Trajectories - CTT Scale', fontsize=14, fontweight='bold')
ax.legend(loc='upper right', framealpha=0.95, fontsize=10)
ax.grid(True, linestyle='--', alpha=0.6)
...
for paradigm in ['IFR', 'ICR', 'IRE']:
    ...
    # Scatter
    paradigm_data = irt_input[irt_input['paradigm'] == paradigm]
    ax.scatter(paradigm_data['TSVR_hours'], paradigm_data['theta'],
               c=color, alpha=RESIDUAL_SCATTER_POINT_ALPHA, s=15, edgecolors='none')

    # CI band
    paradigm_pred = irt_pred_grid[irt_pred_grid['paradigm'] == paradigm]
    ax.fill_between(paradigm_pred['TSVR_hours'], paradigm_pred['ci_lower'], paradigm_pred['ci_upper'],
                    color=color, alpha=MEAN_CI_ALPHA, edgecolor='none')

    # Fitted curve
    ax.plot(paradigm_pred['TSVR_hours'], paradigm_pred['mean'],
...
for paradigm in ['IFR', 'ICR', 'IRE']:
    ...
    # Scatter
    paradigm_data = ctt_input[ctt_input['paradigm'] == paradigm]
    ax.scatter(paradigm_data['TSVR_hours'], paradigm_data['CTT_mean'] * 100,
               c=color, alpha=RESIDUAL_SCATTER_POINT_ALPHA, s=15, edgecolors='none')

    # CI band
    paradigm_pred = ctt_pred_grid[ctt_pred_grid['paradigm'] == paradigm].copy()
    ax.fill_between(paradigm_pred['TSVR_hours'], paradigm_pred['ci_lower'] * 100, paradigm_pred['ci_upper'] * 100,
                    color=color, alpha=MEAN_CI_ALPHA, edgecolor='none')

    # Fitted curve
    ax.plot(paradigm_pred['TSVR_hours'], paradigm_pred['mean'] * 100,
            color=color, linestyle=IRT_LINE_STYLE, linewidth=2.5, label=label)
...
fig.suptitle('RQ 5.3.5: IRT-CTT Convergence - Paradigm-Specific Forgetting Trajectories',
