...
if __name__ == "__main__":
    try:
        ...
        X = regression_input[['RAVLT_T', 'BVMT_T', 'RPM_T']]  # Cognitive test predictors
        ...
        expected_bonf = slope_model_results['p_uncorrected'] * 3  # 3 predictors
        bonf_correct = np.allclose(slope_model_results['p_bonferroni'], expected_bonf, rtol=1e-10)
        if bonf_correct:
            log("[VALIDATION] PASS: Bonferroni correction applied correctly (factor = 3)")
        else:
        ...
        expected_terms = ['intercept', 'RAVLT_T', 'BVMT_T', 'RPM_T']
        actual_terms = slope_model_results['predictor'].tolist()
        if set(actual_terms) == set(expected_terms):
            log("[VALIDATION] PASS: All 4 model terms present (intercept + 3 predictors)")
        else:
