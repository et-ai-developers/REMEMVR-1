...
def compute_effect_sizes(df):
    ...
    for test_name in ['RAVLT', 'BVMT', 'NART', 'RPM']:
    ...
    summary.append(f"Normality satisfied: {n_normal}/4 models")
    summary.append(f"Homoscedasticity satisfied: {n_homosced}/4 models")
...
def bootstrap_analysis(df, n_iterations=2000, seed=42):
    ...
    np.random.seed(seed)
    bootstrap_results = {test: [] for test in ['RAVLT', 'BVMT', 'NART', 'RPM']}
