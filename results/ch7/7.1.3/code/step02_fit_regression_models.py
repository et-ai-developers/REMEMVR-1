...
def fit_domain_model(data, domain_name, predictors=['RAVLT_T', 'BVMT_T', 'RPM_T']):
...
if __name__ == "__main__":
    try:
        ...
        for domain in domains:
            ...
            predictors = ['intercept', 'RAVLT_T', 'BVMT_T', 'RPM_T']
            for i, predictor in enumerate(predictors):
        ...
        log(f"  RAVLT: What β={ravlt_what:.3f} vs Where β={ravlt_where:.3f} {'✓' if ravlt_what > ravlt_where else '✗'}")
        ...
        log(f"  BVMT: Where β={bvmt_where:.3f} vs What β={bvmt_what:.3f} {'✓' if bvmt_where > bvmt_what else '✗'}")
