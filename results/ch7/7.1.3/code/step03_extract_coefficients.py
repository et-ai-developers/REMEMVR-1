...
if __name__ == "__main__":
    try:
        ...
        log(f"{'Domain':<10} {'RAVLT':<10} {'BVMT':<10} {'RPM':<10}")
        log("-" * 40)
        for _, row in matrix_df.iterrows():
            log(f"{row['domain']:<10} {row['RAVLT_T_beta']:>9.3f} {row['BVMT_T_beta']:>9.3f} {row['RPM_T_beta']:>9.3f}")
        ...
        comparisons.append({
            'comparison': 'RAVLT_What - RAVLT_Where',
        ...
        comparisons.append({
            'comparison': 'BVMT_Where - BVMT_What',
        ...
        log(f"[SUMMARY] Extracted 9 beta coefficients (3 predictors × 3 domains)")
