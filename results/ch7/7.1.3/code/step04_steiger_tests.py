...
if __name__ == "__main__":
    try:
        ...
        cog_data = data[['UID', 'RAVLT_T', 'BVMT_T', 'RPM_T']].drop_duplicates()
        ...
        corr_vars = ['What', 'Where', 'When', 'RAVLT_T', 'BVMT_T', 'RPM_T']
        ...
        log(f"  RAVLT-What: r={corr_matrix.loc['RAVLT_T', 'What']:.3f}")
        log(f"  RAVLT-Where: r={corr_matrix.loc['RAVLT_T', 'Where']:.3f}")
        log(f"  RAVLT-When: r={corr_matrix.loc['RAVLT_T', 'When']:.3f}")
        log(f"  BVMT-What: r={corr_matrix.loc['BVMT_T', 'What']:.3f}")
        log(f"  BVMT-Where: r={corr_matrix.loc['BVMT_T', 'Where']:.3f}")
        log(f"  BVMT-When: r={corr_matrix.loc['BVMT_T', 'When']:.3f}")
        log(f"  RPM-What: r={corr_matrix.loc['RPM_T', 'What']:.3f}")
        log(f"  RPM-Where: r={corr_matrix.loc['RPM_T', 'Where']:.3f}")
        log(f"  RPM-When: r={corr_matrix.loc['RPM_T', 'When']:.3f}")
        ...
        bonferroni_alpha = 0.05 / 3  # 3 primary tests
        ...
        for _, row in steiger_df.iterrows():
            ...
            # Check hypothesis
            if 'RAVLT' in row['comparison']:
                hypothesis_met = row['r_diff'] > 0
                log(f"    Hypothesis (RAVLT->What > RAVLT->Where): {'✓' if hypothesis_met else '✗'}")
            elif 'BVMT' in row['comparison']:
                hypothesis_met = row['r_diff'] > 0
                log(f"    Hypothesis (BVMT->Where > BVMT->What): {'✓' if hypothesis_met else '✗'}")
            elif 'RPM' in row['comparison']:
                hypothesis_met = row['r_diff'] < 0.1
                log(f"    Hypothesis (RPM consistent across domains): {'✓' if hypothesis_met else '✗'}")
        
        log("\n[COMPLETE] Step 04 completed successfully")
        log(f"[SUMMARY] Performed 3 Steiger Z-tests with multiple comparison corrections")
