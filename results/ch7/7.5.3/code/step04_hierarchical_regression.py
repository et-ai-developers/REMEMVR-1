    ...
    try:
        ...
        try:
            ...
            for block_name in block_names:
                if block_name in hierarchical_result:
                    block_data = hierarchical_result[block_name]
                    block_results.append({
                        'block': block_name,
                        'r_squared': block_data.get('r_squared', np.nan),
                        'r_squared_change': block_data.get('r_squared_change', np.nan),
                        'f_change': block_data.get('f_change', np.nan),
                        'p_change_uncorrected': block_data.get('p_change', np.nan),
                        'p_change_bonferroni': block_data.get('p_change_bonferroni', np.nan)
                    })
            
            # Get final model coefficients
            if 'final_model' in hierarchical_result:
                final_model = hierarchical_result['final_model']
                coefficients_data = []
                ...
                for i, pred_name in enumerate(predictor_names):
                    ...
                    coefficients_data.append({
                        'predictor': pred_name,
                        'coefficient': params.iloc[param_idx] if param_idx < len(params) else np.nan,
                        'se': stderr.iloc[param_idx] if param_idx < len(stderr) else np.nan,
                        't_statistic': tvalues.iloc[param_idx] if param_idx < len(tvalues) else np.nan,
                        'p_uncorrected': pvalues.iloc[param_idx] if param_idx < len(pvalues) else np.nan,
                        'p_bonferroni': np.nan,  # Will calculate below
            ...
            for i, pred_name in enumerate(predictor_names):
                ...
                if param_idx < len(params):
                    coefficients_data.append({
                        'predictor': pred_name,
                        'coefficient': params.iloc[param_idx],
                        'se': stderr.iloc[param_idx],
                        't_statistic': tvalues.iloc[param_idx],
                        'p_uncorrected': pvalues.iloc[param_idx],
                        'p_bonferroni': min(pvalues.iloc[param_idx] * len(predictor_names), 1.0),
                        'cohens_f2': np.nan  # Will calculate below
                    })
