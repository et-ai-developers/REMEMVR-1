...
# Setup regression
X = df[['age', 'sex', 'education', 'RAVLT_T', 'BVMT_T', 'RPM_T']]
...
for i, predictor in enumerate(X.columns):
    if predictor in ['RAVLT_T', 'BVMT_T', 'RPM_T']:
        p_bonferroni.append(min(p_uncorrected[i] * 3, 1.0))
    else:
...
for i, predictor in enumerate(X.columns):
    ...
    # Report significance
    sig_bonf = p_bonferroni[i] < (0.000597 if predictor in ['RAVLT_T', 'BVMT_T', 'RPM_T'] else 0.05)
