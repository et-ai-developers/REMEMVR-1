...
def compute_slope_contrasts(model, domain_levels):
    ...
    beta_When_TSVR = params['C(paradigm)[T.When]:log_TSVR']
    beta_Where_TSVR = params['C(paradigm)[T.Where]:log_TSVR']

    # Contrast 1: When vs What (already in model as interaction)
    contrast1_est = beta_When_TSVR
    contrast1_se = np.sqrt(cov.loc['C(paradigm)[T.When]:log_TSVR', 'C(paradigm)[T.When]:log_TSVR'])

    # Contrast 2: Where vs What (already in model as interaction)
    contrast2_est = beta_Where_TSVR
    contrast2_se = np.sqrt(cov.loc['C(paradigm)[T.Where]:log_TSVR', 'C(paradigm)[T.Where]:log_TSVR'])
    ...
    var_When = cov.loc['C(paradigm)[T.When]:log_TSVR', 'C(paradigm)[T.When]:log_TSVR']
    var_Where = cov.loc['C(paradigm)[T.Where]:log_TSVR', 'C(paradigm)[T.Where]:log_TSVR']
    cov_When_Where = cov.loc['C(paradigm)[T.When]:log_TSVR', 'C(paradigm)[T.Where]:log_TSVR']
