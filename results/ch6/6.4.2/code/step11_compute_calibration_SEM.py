#!/usr/bin/env python3
"""
RQ 6.4.2 - Paradigm Calibration SEM Implementation
===================================================

CONTEXT:
--------
Original analysis reported Paradigm main effect χ²(2)=7.83, p=0.040 Bonferroni,
BUT difference score reliability r_diff=0.66 (MARGINAL, below 0.70 threshold).
Effect sizes TRIVIAL (d<0.11 for all pairwise contrasts).

BLOCKER: Issue 002 - Marginal reliability prevents FULL PLATINUM certification.

GOAL:
-----
Apply paradigm-stratified SEM to compute latent calibration scores,
improve reliability to r≥0.70, and test if Paradigm main effect survives
POST-SEM analysis (ROBUST) or disappears (SPURIOUS).

METHODOLOGY:
------------
1. Load paradigm-stratified calibration data (1200 rows: 100 UID × 4 tests × 3 paradigms)
2. Re-standardize theta scores BY Paradigm (critical for stratified analysis)
3. Compute ICC-based reliability BY Paradigm (between-person vs within-person variance)
4. Apply SEM latent difference model SEPARATELY for each paradigm
...
PRECEDENT:
----------
- RQ 6.3.2 (Domain stratification: What/Where/When, r_diff=-0.079 → r=0.877)
- RQ 6.8.2 (LocationType stratification: Source/Dest, r_diff=-0.168 → r=0.830)
Both successfully improved CATASTROPHIC negative reliabilities to EXCELLENT.

EXPECTED OUTCOME:
-----------------
r_diff=0.66 (MARGINAL) → r≥0.70 (ACCEPTABLE) or r≥0.80 (EXCELLENT)
Paradigm effect may SURVIVE (ROBUST) or DISAPPEAR (SPURIOUS) POST-SEM.
...
# Configure logging
log_file = Path("results/ch6/6.4.2/logs/step11_SEM.log")
...
# File paths
INPUT_FILE = Path("results/ch6/6.4.2/data/step00_calibration_by_paradigm.csv")
OUTPUT_FILE = Path("results/ch6/6.4.2/data/step11_calibration_scores_SEM.csv")
DIAGNOSTICS_FILE = Path("results/ch6/6.4.2/data/step11_SEM_diagnostics.csv")
...
def fit_sem_latent_difference(df: pd.DataFrame, paradigm: str) -> Tuple[pd.Series, Dict]:
    """
    Fit SEM latent difference model for a single paradigm.
    ...
    ----------
    df : pd.DataFrame
        Data for single paradigm (must contain UID, theta_accuracy_z, theta_confidence_z)
    paradigm : str
...
def main():
    """
    Main execution: Paradigm-stratified SEM for RQ 6.4.2.
    """
    logger.info("="*80)
    logger.info("RQ 6.4.2 - Paradigm Calibration SEM Implementation")
    logger.info("="*80)
    ...
    logger.info(f"Paradigms: {df['Paradigm'].unique()}")
    ...
    assert df['Paradigm'].nunique() == 3, f"Expected 3 paradigms, got {df['Paradigm'].nunique()}"
    ...
    # GLOBAL standardization FIRST (preserve paradigm differences for LMM)
    logger.info("\nGlobal standardization (across all paradigms)...")
    ...
    # Process each paradigm separately FOR ICC COMPUTATION ONLY
    for paradigm in sorted(df['Paradigm'].unique()):
        logger.info(f"\n{'='*80}")
        logger.info(f"Processing Paradigm: {paradigm}")
        logger.info(f"{'='*80}")

        # Subset data
        df_paradigm = df[df['Paradigm'] == paradigm].copy()
        logger.info(f"N = {len(df_paradigm)} observations ({df_paradigm['UID'].nunique()} UIDs × {df_paradigm['TEST'].nunique()} tests)")

        # Re-standardize theta scores BY paradigm (ONLY for ICC computation)
        logger.info("\nRe-standardizing theta scores BY paradigm (for ICC only)...")
        df_paradigm['theta_accuracy_z_icc'] = (
            (df_paradigm['theta_accuracy'] - df_paradigm['theta_accuracy'].mean()) /
            df_paradigm['theta_accuracy'].std()
        )
        df_paradigm['theta_confidence_z_icc'] = (
            (df_paradigm['theta_confidence'] - df_paradigm['theta_confidence'].mean()) /
            df_paradigm['theta_confidence'].std()
        )
        logger.info(f"  theta_accuracy_z_icc: mean={df_paradigm['theta_accuracy_z_icc'].mean():.4f}, "
                   f"sd={df_paradigm['theta_accuracy_z_icc'].std():.4f}")
        logger.info(f"  theta_confidence_z_icc: mean={df_paradigm['theta_confidence_z_icc'].mean():.4f}, "
                   f"sd={df_paradigm['theta_confidence_z_icc'].std():.4f}")

        # Use GLOBAL standardization for SEM (preserves paradigm differences)
        df_paradigm['theta_accuracy_z'] = df_paradigm['theta_accuracy_z_global']
        df_paradigm['theta_confidence_z'] = df_paradigm['theta_confidence_z_global']

        # PRE-SEM: Compute ICC-based reliability (using within-paradigm standardization)
        logger.info("\n=== PRE-SEM Reliability (ICC-based) ===")
        r_xx = compute_icc(df_paradigm, 'theta_accuracy_z_icc')
        r_yy = compute_icc(df_paradigm, 'theta_confidence_z_icc')
        r_xy = df_paradigm.groupby('UID')[['theta_accuracy_z_icc', 'theta_confidence_z_icc']].mean().corr().iloc[0, 1]
        ...
        latent_calibration, sem_diagnostics = fit_sem_latent_difference(df_paradigm, paradigm)
        df_paradigm['latent_calibration'] = latent_calibration

        # POST-SEM: Validate with split-half reliability
        r_half, r_full = compute_split_half_reliability(df_paradigm, 'latent_calibration')

        # Correlation with simple difference (validation)
        df_paradigm['simple_diff'] = df_paradigm['theta_confidence_z'] - df_paradigm['theta_accuracy_z']
        corr_with_simple = df_paradigm.groupby('UID')[['latent_calibration', 'simple_diff']].mean().corr().iloc[0, 1]
        ...
        diagnostics_row = {
            'paradigm': paradigm,
            'n_obs': len(df_paradigm),
            'n_uid': df_paradigm['UID'].nunique(),
            # PRE-SEM
            'pre_r_xx': r_xx,
            'pre_r_yy': r_yy,
            'pre_r_xy': r_xy,
        ...
        df_paradigm_out = df_paradigm[[
            'UID', 'TEST', 'Paradigm', 'TSVR_hours',
            'theta_accuracy', 'theta_confidence',
            'theta_accuracy_z', 'theta_confidence_z',
            'latent_calibration'
        ]].copy()
        all_latent_scores.append(df_paradigm_out)

    # Combine results
    logger.info(f"\n{'='*80}")
    logger.info("Combining results across paradigms...")
    logger.info(f"{'='*80}")
    ...
    for _, row in df_diagnostics.iterrows():
        logger.info(f"{row['paradigm']:8s} | {row['pre_r_diff']:10.4f} | "
                   f"{row['post_r_full']:11.4f} | {row['improvement_pp']:+10.1f} pp | "
                   f"{row['post_classification']}")

    logger.info(f"\n{'='*80}")
    logger.info("✓ RQ 6.4.2 SEM Implementation Complete")
    logger.info(f"{'='*80}")
