...
def main():
    ...
    accuracy_path = Path("/home/etai/projects/REMEMVR/results/ch5/5.3.1/data/step01_irt_input_paradigm.csv")
    if not accuracy_path.exists():
        logger.error(f"EXPECTATIONS ERROR: Accuracy source missing: {accuracy_path}")
        return
    
    # Ch6 6.4.1: Confidence by paradigm (item-level)
    confidence_path = Path("/home/etai/projects/REMEMVR/results/ch6/6.4.1/data/step01_irt_input_paradigm.csv")
    ...
    logger.info(f"Accuracy: {len(acc_df)} items")
    logger.info(f"Confidence: {len(conf_df)} items")
